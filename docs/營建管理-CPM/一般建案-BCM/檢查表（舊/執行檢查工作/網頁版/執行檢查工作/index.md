...
{% tabs %}
{% tab title="檢查位置(Location)" %}
系統會預設帶入專案的建地結構，但若有該檢查有關閉『[限定建地結構資料](../../../jian-cha-liu-cheng-she-ding#xian-ding-jian-di-jie-gou-zi-liao)』，則建議在此細化至具體構件，例如「3F-B區樑柱接頭」，確保查驗點位精確。
{% endtab %}

{% tab title="檢查標準(Standard)" %}
顯示該項目的技術規範。執行人員可在此比對實測數據，例如標準為「垂直度 $$\pm$$ 3mm」，可於描述欄填寫實測結果，落實數據化管理。
{% endtab %}

{% tab title="描述(Description)" %}
用於記錄現場執行細節。

> 範例： 「經現場實測，鋼筋間距誤差控制在 5mm 內，且箍筋彎鉤角度均符合 135 度要求，無鬆脫現象。」
{% endtab %}

{% tab title="附圖(Photo Evidence)" %}
現場拍攝之缺失照或合格存證照。

App 系統支援自動帶入時間、座標、工程名稱、施工位置之浮水印，確保照片的真實性，符合營建業品管規範（如公共工程委員會之照片紀錄要求）。
{% endtab %}

{% tab title="附件(Attachments)" %}
...
有關檢查位置填寫方式設定，請參閱 ➙ [限定建地結構資料](../../../jian-cha-liu-cheng-she-ding#xian-ding-jian-di-jie-gou-zi-liao)
{% endtab %}
...
<div>![圖十 - 填寫檢查紀錄(一)](../../../../../../images/d91077c39f97a2af.png) ![圖十一 - 填寫檢查紀錄(二)](../../../../../../images/07401e9dd814a80a.png) ![圖 - 填寫檢查紀錄(三)](../../../../../../images/872559a3c2410f6e.png)</div>
...
確認所有檢查項目皆已填寫完畢，且紀錄確實無誤後，即可點選<kbd><mark style="color:purple;">**初驗完成，送交審核**<mark style="color:purple;"></kbd>。系統將依據該檢查所使用之『檢查流程』，將該筆紀錄即時傳送給相關管理人員（如：工地主任或專案經理）進行評估與簽認。
...
2. 此時，審核人員（如工地主任或專案經理）的待辦清單中會出現該筆任務，並查看您上傳的所有照片、圖面標示及實測數據。並獲得核定、退回的權限。

（二）在審核過程中，若審核人員（如工地主任或 PM）認為檢查紀錄不夠詳盡、照片不清晰或判定結果有誤，可執行****退回****操作：
